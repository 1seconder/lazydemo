...
function closeClear(){
	...
	$("#shangQuanSelect-1").append("<option value='hhh'>请选择商圈</option>");
	$("#buildingSelect-2").append("<option value='hhh'>请选择楼盘</option>");
...
function queryHeadMessage(){
	...
	$("#headMessage").html("<p style='color:black'>当前管辖共"+all+"个楼盘，共指派"+personNum+"个责任人，其中"+(result.responseJSON.result2.length)+" 个楼盘（"+result.responseJSON.result2[0].lp_name+"、"+result.responseJSON.result2[1].lp_name+"）未指派</p>");
//	$("#headMessage").append("<button class='btn btn-secondary' onclick='jQuery(\"#modal-5\").modal(\"show\", {backdrop: \"fade\"});'>参数配置</button>");
}

function setParameter(){
	var parameter = $("#setParameter").val();
	if($.trim(parameter) == ""){
		alert("请输入数字");
		return;
	}
	var requestParameter = new Object();
	requestParameter.parameter = parameter;
	var dataJson = JSON.stringify(requestParameter);
	var url = getRootPath()+"/services/assignResponsiblePersonService/setParameter";
	var result = $.ajax({
		url : url,
		data : dataJson,
		type : "POST",
