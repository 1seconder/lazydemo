...
import com.newenv.communityFocus.util.HttpClientUtil;
...
//			dataMap.put("cus", cus);
	...
	public Response saveFwBasicInfo(HouseBasicInfo house) {
		...
		try{
			...
			if(house.getStage() == 1){
				stage = "接触期";
			}else if(house.getStage() == 2){
				stage = "客户带看";
			}else if(house.getStage() == 3){
				stage = "合同洽谈";
			}
			...
			String msg = stage +","+string + "保存房屋基本信息，房屋："+ getFw(house.getFhid());
			updatePeriod(house.getFhid(), msg,stage,deptId, userId);
...
//		接触    委托   了解需求  客户带看    合同洽谈
	...
	public Response saveCustomerInfo2(JjrCustomerParams info) {
		...
		try{
			...
			if(info.getStage() == 1){
				stage = "接触期";
			}else if(info.getStage() == 2){
				stage = "客户带看";
			}else if(info.getStage() == 3){
				stage = "合同洽谈";
			}
			if(info.getType() == 1){//add
				...
				String string = userName.get(0).getString("fullname");
				//7跟进情况
				String msg = stage +","+string + "保存经纪人业主信息，房屋："+ getFw(info.getFhid());
				PeriodInfo period = daoHelper.getObjectByPOJO(PeriodInfo.class, Cnd.where("fhid", "=", info.getFhid()).and("creatorid","=",creatorId).and("stage","=",stage));
				if(period == null){
...
//			Trans.close();
	...
	public Response saveTelInfo(TelFollow info) {
		...
		try{
			...
			List<Record> userName = userDao.getUserName(creatorId);
			String string = userName.get(0).getString("fullname");
			String msg = stage +","+string + "保存电话跟进信息，房屋："+ getFw(info.getFhid());
			updatePeriod(info.getFhid(),msg,stage,bmId, creatorId);
			saveOperationLog(info.getFhid(), msg);
		}catch(Exception e){
			logger.error(e.getMessage());
			return Response.status(500).build();
	...
	public Response saveCompetition(Competition info) {
		...
		try{
			...
			List<Record> userName = userDao.getUserName(creatorId);
			String string = userName.get(0).getString("fullname");
			String msg = stage +","+string + "保存竞争对手，房屋："+ getFw(info.getFhid());
			updatePeriod(info.getFhid(),msg,stage, bmId , creatorId);
			saveOperationLog(info.getFhid(), msg);
		}catch(Exception e){
			logger.error(e.getMessage());
	...
	@Produces({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	@Consumes({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	@Path("/saveLoanRecords")
	public Response saveLoanRecords(TelFollow info) {
		List<Record> list = new ArrayList<Record>();
		try{
			int bmId,creatorId;
			if(info.getCreatorId() != 0){
			...
			List<Record> userName = userDao.getUserName(creatorId);
			String string = userName.get(0).getString("fullname");
			String msg = stage +","+string + "进行快速贷款申请";
			updatePeriod(info.getFhid(),msg,stage,bmId, creatorId);
			saveOperationLog(info.getFhid(), msg);
		}catch(Exception e){
			logger.error(e.getMessage());
	...
	@Produces({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	@Consumes({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	@Path("/queryPhpProvinceList")
	public Response queryPhpProvinceList() {
		List<Map<String,String>> result = new ArrayList<Map<String,String>>();
		Map<String,String> hashMap = new HashMap<String,String>();
		try{
			String str = HttpClientUtil.getInstance().post(SecurityUserHolder.getConfig().getJmbjrUrl()+"borrow-java_selectpro", "UTF-8",hashMap);
			result  = (List<Map<String, String>>) JSONArray.parse(str.substring(1,str.length()-1));
		}catch(Exception e){
			logger.error(e.getMessage());
			return Response.status(500).build();
		}
		...
		resultFeedback.setListJson(result);
	...
	@Produces({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	@Consumes({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	@Path("/queryPhpCityList")
	public Response queryPhpCityList(@QueryParam ("pId") String pId) {
		List<Map<String,String>> result = new ArrayList<Map<String,String>>();
		try{
			String str = HttpClientUtil.getInstance().get(SecurityUserHolder.getConfig().getJmbjrUrl()+"borrow-java_selectarea?pid="+pId, "UTF-8");
			result  = (List<Map<String, String>>) JSONArray.parse(str.substring(1,str.length()-1));
		}catch(Exception e){
			logger.error(e.getMessage());
			return Response.status(500).build();
	...
	public Response updatePeriod(@PathParam("fhId") long fhId,@PathParam("msg") String msg,@PathParam("stage") String stage,@PathParam("deptId") int deptId, @PathParam("creatorId") int creatorId){
		...
		PeriodInfo period = daoHelper.getObjectByPOJO(PeriodInfo.class, Cnd.where("fhid", "=", fhId).and("creatorid","=",creatorId).and("stage","=",stage));
		if(period == null){
			PeriodInfo periodInfo = PeriodInfo.getPeriodInfo();
...
//				dataMap.put("cus", cus);
	...
	@Produces({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	@Consumes({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	@Path("/updatePeriodAndOperation/{creatorId}/{fhId}/{deptId}/{operationType}")
	public void updatePeriodAndOperation(@PathParam("creatorId") Integer creatorId, @PathParam("fhId") Integer fhId,
			@PathParam("deptId") Integer deptId, @PathParam("operationType") Integer operationType) {
		try {
			List<Record> userName = userDao.getUserName(creatorId);
			String string = userName.get(0).getString("fullname");
			String msg = "";
			...
			msg = string + msg;
			updatePeriod(fhId, msg, "委托期", deptId, creatorId);

			OperationLog operationLog = new OperationLog(creatorId, deptId);
			operationLog.setFhid(fhId);
			operationLog.setMessage(msg);
			operationLog.setType(11);
			daoHelper.save(operationLog);
		} catch (Exception e) {
			logger.error(e.getMessage());
			e.printStackTrace();
		}
