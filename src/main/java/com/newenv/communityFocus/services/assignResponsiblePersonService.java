...
public class assignResponsiblePersonService extends BaseService {
	...
	public Response assignResponsiblePerson(RequestParameter requestParameter) {
		Integer result = 0;
		Integer num = 5;
		if (daoHelper.getList("select number from lp_syscs where name = '责任人分派参数'").size() > 0) {
			String numStr = daoHelper.getList("select number from lp_syscs where name = '责任人分派参数'").get(0)
					.get("number") == null ? ""
							: daoHelper.getList("select number from lp_syscs where name = '责任人分派参数'").get(0)
									.get("number").toString();
			if (StringUtils.notEmpty(numStr)) {
